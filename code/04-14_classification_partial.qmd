...
weather_class <- weather %>%
  mutate(strength_gtex_class = cut_number(strength_gtex,
                                          n = 3,
                                          labels = c("Low",
                                                     "Medium",
                                                 "High")))%>%
  dplyr::select(year, site, strength_gtex, strength_gtex_class, everything())
...
ggplot() +
  ...
  geom_density(data = weather_test, 
               aes(x = strength_gtex),
               color = "blue") +
facet_wrap(~strength_gtex_class)
...
mtry: 35  
trees: 149  
Notes: If using an equal quantile like us then it will equally classify the observation which is why accuracy works.
...
final_fit %>%
  collect_metrics() 
```
Let's hand-calculate accuracy: diagonal divided by everthing else 
```{r}
...
final_spec %>%
  ...
  augment(new_data = bake(weather_prep, 
                          weather_train)) %>%
  accuracy(truth = strength_gtex_class,
           estimate = .pred_class)
